...
export const useReportUpload = () => {
  ...
  useEffect(() => {
    if (letterGenerated) {
      ...
      if (pendingLetter || generatedLetters) {
        ...
        // Enable test mode subscription to allow viewing the letters before subscription
  ...
  const handleGenerateDispute = (account?: CreditReportAccount) => {
    if (reportData) {
      ...
      // Enable test mode subscription to allow viewing the letters without subscription
      sessionStorage.setItem('testModeSubscription', 'true');
