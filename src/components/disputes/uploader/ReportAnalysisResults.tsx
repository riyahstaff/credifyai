    ...
    if (reportData) {
      ...
      try {
        ...
        if (generatedLetters && generatedLetters.length > 0) {
          ...
          // Create a simplified letter if we need to - just to ensure we have something
          if (generatedLetters[0].content.length < 50) {
            console.log("First letter content is too short, creating a more detailed letter");
            generatedLetters[0].content = `
Dear ${generatedLetters[0].recipient},

I am writing to dispute the following information in my credit report:

Account Name: ${generatedLetters[0].accountName || "Unknown"}
Account Number: ${generatedLetters[0].accountNumber || "Unknown"}
Issue: ${generatedLetters[0].errorType || "Inaccurate Information"}

I am disputing this information because it is inaccurate and is negatively affecting my credit score.

Under the Fair Credit Reporting Act (FCRA), you are required to investigate this dispute and remove any inaccurate information.
...
[YOUR NAME]
            `;
            // Also set letterContent
            generatedLetters[0].letterContent = generatedLetters[0].content;
          }
          ...
          // Also store the first letter separately for quick access
          sessionStorage.setItem('pendingDisputeLetter', JSON.stringify(generatedLetters[0]));
          
          // Set the flag to indicate auto-generated letters
          ...
          // Navigate after a delay to ensure storage operations complete
          setTimeout(() => {
...
[YOUR NAME]
          ...
          console.log("Fallback letter created and stored");
...
[YOUR NAME]
        ...
        // Store the fallback letter
        sessionStorage.setItem('generatedDisputeLetters', JSON.stringify([errorFallbackLetter]));
        sessionStorage.setItem('pendingDisputeLetter', JSON.stringify(errorFallbackLetter));
        sessionStorage.setItem('autoGeneratedLetter', 'true');
...
[YOUR NAME]
        ...
        // Store the letter directly
        sessionStorage.setItem('generatedDisputeLetters', JSON.stringify([directLetter]));
        sessionStorage.setItem('pendingDisputeLetter', JSON.stringify(directLetter));
        sessionStorage.setItem('autoGeneratedLetter', 'true');
        ...
        console.log("- autoGeneratedLetter exists:", !!sessionStorage.getItem('autoGeneratedLetter'));
...
[YOUR NAME]
        ...
        sessionStorage.setItem('generatedDisputeLetters', JSON.stringify([recoveryLetter]));
        sessionStorage.setItem('pendingDisputeLetter', JSON.stringify(recoveryLetter));
        sessionStorage.setItem('autoGeneratedLetter', 'true');
