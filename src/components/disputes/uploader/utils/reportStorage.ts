...
export const storeReportData = (reportData: CreditReportData): void => {
  try {
    if (!reportData) {
      console.error('Attempted to store undefined or null report data');
      return;
    }
    sessionStorage.setItem('creditReportData', JSON.stringify(reportData));
    console.log('Credit report data stored in session storage', reportData);
  } catch (error) {
...
export const getStoredReportData = (): CreditReportData | null => {
  try {
    ...
    if (storedData) {
      ...
      console.log('Retrieved credit report data from session storage', parsedData);
      return parsedData;
    }
  } catch (error) {
    console.error('Error retrieving credit report data:', error);
  }
  console.log('No credit report data found in session storage');
  return null;
};
...
export const storeAccountForDispute = (account: CreditReportAccount): void => {
  try {
    if (!account) {
      console.error('Attempted to store undefined or null account');
      return;
    }
    
    // Safely access the optional bureau property
    const bureau = account.bureau || 
      (account.bureauReporting && account.bureauReporting.length > 0 ? 
        account.bureauReporting[0] : 'Experian');
...
export const storeGeneratedLetters = (letters: any[]): void => {
  try {
    if (!letters || !Array.isArray(letters)) {
      console.error('Attempted to store invalid letters data:', letters);
      return;
    }
