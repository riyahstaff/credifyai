...
export const useDisputeLettersData = () => {
  ...
  useEffect(() => {
    ...
    const checkForAutoGeneratedLetters = () => {
      ...
      if (autoGeneratedFlag === 'true' && generatedLetters) {
        try {
          const disputeLetters = JSON.parse(generatedLetters);
          console.log(`Found ${disputeLetters.length} auto-generated letters`);
          
          if (disputeLetters.length === 0) {
            console.log("No letters found in generatedDisputeLetters");
            return;
          }
          
          const newLetters: Letter[] = disputeLetters.map((disputeData: any, index: number) => ({
            id: Date.now() + index,
          ...
          if (newLetters.length > 0) {
            setLetters(prevLetters => [...newLetters, ...prevLetters]);
            ...
            toast({
              title: "AI-Generated Letters Ready",
              description: `CLEO has generated ${newLetters.length} dispute letters based on your credit report analysis.`,
              duration: 5000,
            });
            
            if (user?.id) {
              disputeLetters.forEach((disputeData: any) => {
                saveLetter(disputeData);
              });
            }
          }
        } catch (error) {
          console.error('Error processing auto-generated letters:', error);
        }
      } 
      else if (autoGeneratedFlag === 'true' && pendingLetter) {
        try {
          const disputeData = JSON.parse(pendingLetter);
          console.log("Found single auto-generated letter:", disputeData.accountName);
          
          if (!disputeData.letterContent) {
            console.error("No letter content in pendingDisputeLetter");
            toast({
              title: "Letter Generation Failed",
              description: "The generated letter is missing content. Please try again.",
              variant: "destructive",
              duration: 5000,
            });
            return;
          ...
          toast({
            title: "Error Processing Letter",
            description: "There was a problem with the generated letter. Please try again.",
            variant: "destructive",
            duration: 5000,
          });
        }
      } else if (autoGeneratedFlag === 'true') {
        console.log("Auto-generated flag is true but no letters found");
        sessionStorage.removeItem('autoGeneratedLetter');
      }
    };
    
    checkForAutoGeneratedLetters();
    const timer = setTimeout(checkForAutoGeneratedLetters, 1000);
