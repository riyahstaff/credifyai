...
export function useDisputeLettersData(testMode: boolean = false) {
  ...
  useEffect(() => {
        ...
        if (generatedLettersJSON) {
          try {
            ...
            if (Array.isArray(parsedLetters) && parsedLetters.length > 0) {
              ...
              // Select the first letter by default if none selected
    ...
    // Update profile information before loading letters
    if (profile) {
      console.log("Found profile in auth context, storing for letter generation:", profile.full_name);
    ...
    // Force clearing of old cached letters with placeholder data
    const shouldClearOldLetters = () => {
      const storedLetters = sessionStorage.getItem('generatedDisputeLetters');
      if (storedLetters) {
        try {
          const letters = JSON.parse(storedLetters);
          // Check if any letter contains placeholder data
          return letters.some((letter: any) => 
            letter.content && (
              letter.content.includes('[YOUR NAME]') ||
              letter.content.includes('[YOUR ADDRESS]') ||
              letter.content.includes('UNKNOWN ACCOUNT')
            )
          );
        } catch (e) {
          return false;
    ...
    // Clear old placeholder letters if found
    if (shouldClearOldLetters()) {
      console.log("Clearing old letters with placeholder data");
    ...
    // Prevent infinite loops - only load once per session
    const lettersLoaded = sessionStorage.getItem('lettersAlreadyLoaded');
    if (lettersLoaded === 'true') {
  ...
  }, [toast, testMode, profile]); // Removed selectedLetter and location.pathname to prevent loops
