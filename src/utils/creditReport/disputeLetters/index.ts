...
export async function generateLettersForIssues(
  ...
  for (const issue of issues) {
    try {
      ...
      const account = issue.account || findAccountInReportData(issue.accountId, reportData);
      ...
      const accountName = account?.accountName || issue.accountName || "Account in question";
...
function findAccountInReportData(accountId: string | undefined, reportData: CreditReportData) {
  ...
  if (accountId) {
    const accountById = reportData.accounts.find(acc => acc.id === accountId);
